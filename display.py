...
class FieldAnalyzer(Process):
    ...
    def run(self):
        now_time = time.time()
        while True:
            img = self.cam.getImage().regionSelect(self.crop_points[0], self.crop_points[1], self.crop_points[2], self.crop_points[3]).warp(self.field_crop_boundary)
            ...
            if self.debug:
                ...
                if blobs:
                    blobs.draw(width=4)
                    print "FPS: " + str(fps) + "Puck Locations: " + str(self.puckLocations()) + " Percent progression: " + str(self.puckLocationsPercent()) 
                img.show()
    ...
    def puckLocationsPercent(self):
        motorA = ((self.field_post_crop_limits[1] - self.field_post_crop_limits[0]) - \
                  (self.field_post_crop_limits[1] - self.puck_locations[0].x)) / \
                 float(self.field_post_crop_limits[1] - self.field_post_crop_limits[0])

        motorB = ((self.field_post_crop_limits[1] - self.field_post_crop_limits[0]) - \
                  (self.field_post_crop_limits[1] - self.puck_locations[1].x)) / \
                 float(self.field_post_crop_limits[1] - self.field_post_crop_limits[0])
    ...
    def calibrate(self):
        print "We are displaying a live feed from the cam.  Click on a point of the field we tell you, then enter that info\n\n"
        ...
        bottom_right = tuple(int(v) for v in re.findall("[0-9]+", bottom_right))
        ...
        self.field_crop_boundary.append((bottom_left[0] - locations[0], top_left[1] - locations[1]))
        self.field_crop_boundary.append((bottom_right[0] - locations[0], top_right[1] - locations[1]))
        self.field_crop_boundary.append((top_right[0] - locations[0], bottom_right[1] - locations[1]))
        self.field_crop_boundary.append((top_left[0] - locations[0], bottom_left[1] - locations[1]))
        ...
        while inVal != -1:
            ...
            img = self.cam.getImage().regionSelect(locations[0], locations[1], locations[2], locations[3]).warp(self.field_crop_boundary)
            ...
            inVal = int(raw_input("Enter new thresh val for blobs, then -1 to confirm lighting calibration: "))
            print "\n"


        temp_positions = [0, 0, 0, 0]
        inVal = ""
        print "We are now taking some simple measurements of the post-cropped playing field."
        raw_input("Place the puck in the upper-left most side of the field and press [Enter]")
        while not re.match(r"[yY]", inVal):
            ...
            img.show()
            inVal = str(raw_input("Enter y/Y if the puck is selected, and the displayed coordinate appears reasonable, otherwise just hit [Enter]"))
            print inVal


        inVal = ""
        raw_input("Place the puck in the upper-right most side of the field and press [Enter]")
        while not re.match(r"[yY]", inVal):
            ...
            img.show()
            inVal = raw_input("Enter y/Y if the puck is selected, and the displayed coordinate appears reasonable, otherwise just hit [Enter]")
        
        inVal = ""
        raw_input("Place the puck in the bottom-left most side of the field and press [Enter]")
        while not re.match(r"[yY]", inVal):
        ...
        inVal = ""
        raw_input("Place the puck in the bottom-right most side of the field and press [Enter]")
        while not re.match(r"[yY]", inVal):
